 ...
 */
public class FunWaveView extends View {
    ...
    public FunWaveView(Context context) {
        this(context, null);
    }

    public FunWaveView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FunWaveView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.FunWaveView, defStyle, 0);
        mWaveColor = a.getColor(
                R.styleable.FunWaveView_waveColor,
                mWaveColor);
        mBaseLineColor = a.getColor(
                R.styleable.FunWaveView_baselineColor,
                mBaseLineColor);

        waveStrokeWidth = a.getDimension(
                R.styleable.FunWaveView_waveStokeWidth,
                waveStrokeWidth);

        max = (short) a.getInt(R.styleable.FunWaveView_maxValue, max);
        invalidateTime = a.getInt(R.styleable.FunWaveView_invalidateTime, invalidateTime);

        space = a.getDimension(R.styleable.FunWaveView_space, space);
        a.recycle();
        initPainters();
